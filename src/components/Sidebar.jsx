...
export default function Sidebar({ lessons, currentLessonId, onSelectLesson, isOpen, setIsOpen }) {
  return (
    <div className={`bg-white/60 backdrop-blur-xl border-slate-200 h-screen overflow-hidden flex flex-col no-print sticky top-0 shadow-lg shrink-0 transition-all duration-300 ease-in-out ${isOpen ? 'w-72 border-r' : 'w-0 border-r-0'}`}>
      <div className="w-72 flex flex-col h-full overflow-y-auto">
        <div className="p-6 border-b border-slate-200/50 flex justify-between items-center shrink-0">
          ...
          <button onClick={() => setIsOpen(false)} className="text-slate-400 hover:text-slate-700 bg-slate-100 hover:bg-slate-200 p-1.5 rounded-full transition-colors" title="收折側邊欄">
            <ChevronLeft size={20} />
          </button>
        </div>
        ...
        <div className="p-6 border-t border-slate-200/50 bg-slate-50/50 mt-auto shrink-0">
